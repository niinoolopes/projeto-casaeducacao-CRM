...
gulp.task('watch', () => {
    ...
    gulp.watch(`${root.dev}/${css.path}/${css.src}/${css.allFile}`, ['styles', 'styles-min']);
    gulp.watch(`${root.dev}/${javascript.path}/${javascript.src}/${javascript.allFile}`, ['scripts', 'scripts-min']);
});
...
gulp.task('bootstrap-js', () => {
    gulp.src(`${root.dev}/${files.path}/bootstrap/js/dist/**/*.js`)
        ...
        .pipe(uglify())
        .pipe(gulp.dest(`${root.dev}/${javascript.path}`))
    console.log(`bootstrap-min.js OK`)
});
